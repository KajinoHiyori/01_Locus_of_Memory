...
int g_nCntCameraRoll;			// 回り込みカウント
int g_EventStateCounter;
...
void UpdateGameCamera(void)
{
	...
	for (int nCntCamera = 0; nCntCamera < g_nNumCamera; nCntCamera++, pCamera++, pPlayer++)
	{
		if (*pEventState == EVENTSTATE_NORMAL || *pEventState == EVENTSTATE_SUNSETDELAY)
		{
			...
			pCamera->posRDest.y = pPlayer->pos.y + UPVEIW;
		...
		else if (*pEventState == EVENTSTATE_LOOKDOWN)
		{
			g_EventStateCounter--;
