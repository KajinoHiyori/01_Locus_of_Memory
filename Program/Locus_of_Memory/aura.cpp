...
Aura g_ameshcylinder[MAX_AURA];					// オーラの情報
...
void InitAura(void)
{
	...
	{
		g_ameshcylinder[nCntAura].pVtxBuff = NULL;
		g_ameshcylinder[nCntAura].pIdxBuff = NULL;
		g_ameshcylinder[nCntAura].pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		g_ameshcylinder[nCntAura].rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		g_ameshcylinder[nCntAura].fRadius = AURA_RADIUS + (nCntAura * 15.0f);
		g_ameshcylinder[nCntAura].fAngle = ((float)(rand() % 629 - 314) / 100);
		g_ameshcylinder[nCntAura].bUse = false;
		...
		pDevice->CreateVertexBuffer(sizeof(VERTEX_3D) * AURA_SPLIT_WIDHT * AURA_SPLIT_HEIGTH,
			...
			FVF_VERTEX_3D,
			D3DPOOL_MANAGED,
			&g_ameshcylinder[nCntAura].pVtxBuff,
			NULL);
	}
	...
	for (int nCntAura = 0; nCntAura < MAX_AURA; nCntAura++)
	{
		...
		g_ameshcylinder[nCntAura].pVtxBuff->Lock(0, 0, (void**)&pVtx, 0);
		...
		for (int nCntHeight = 0; nCntHeight < AURA_SPLIT_HEIGTH; nCntHeight++)
		{
			...
			{
				// 頂点座標の設定
				pVtx[nCntVtxWidht].pos = D3DXVECTOR3(sinf(fRot) * g_ameshcylinder[nCntAura].fRadius, -AURA_HEIGHT * nCntHeight, cosf(fRot) * g_ameshcylinder[nCntAura].fRadius);
		...
		// 頂点バッファをアンロックする
		g_ameshcylinder[nCntAura].pVtxBuff->Unlock();
	}
	...
	for (int nCntAura = 0; nCntAura < MAX_AURA; nCntAura++)
	{
		...
		pDevice->CreateIndexBuffer(sizeof(WORD) * (AURA_SPLIT_WIDHT * 2 * (AURA_SPLIT_HEIGTH - 1) + ((AURA_SPLIT_HEIGTH - 2) * 2)),
			...
			D3DFMT_INDEX16,
			D3DPOOL_MANAGED,
			&g_ameshcylinder[nCntAura].pIdxBuff,
			NULL);

		WORD* pIdx;		// インデックス情報へのポインタ

		// インデックスバッファをロックし、頂点情報へのポインタを取得
		g_ameshcylinder[nCntAura].pIdxBuff->Lock(0, 0, (void**)&pIdx, 0);
		...
		// インデックスバッファをアンロックする
		g_ameshcylinder[nCntAura].pIdxBuff->Unlock();
	}
...
void UninitAura(void)
{
	...
	{
		// 頂点バッファの破棄
		if (g_ameshcylinder[nCntAura].pVtxBuff != NULL)
		{
			g_ameshcylinder[nCntAura].pVtxBuff->Release();
			g_ameshcylinder[nCntAura].pVtxBuff = NULL;
		}

		// インデックスバッファの破棄
		if (g_ameshcylinder[nCntAura].pIdxBuff != NULL)
		{
			g_ameshcylinder[nCntAura].pIdxBuff->Release();
			g_ameshcylinder[nCntAura].pIdxBuff = NULL;
		}
	}
}
...
void DrawAura(void)
{
	...
	{
		// ワールドマトリックスの初期化
		D3DXMatrixIdentity(&g_ameshcylinder[nCntAura].mtxWorld);

		// 向きを反映
		D3DXMatrixRotationYawPitchRoll(&mtxRot, g_ameshcylinder[nCntAura].rot.y, g_ameshcylinder[nCntAura].rot.x, g_ameshcylinder[nCntAura].rot.z);
		D3DXMatrixMultiply(&g_ameshcylinder[nCntAura].mtxWorld, &g_ameshcylinder[nCntAura].mtxWorld, &mtxRot);

		// 位置を反映
		D3DXMatrixTranslation(&mtxTrans, g_ameshcylinder[nCntAura].pos.x, g_ameshcylinder[nCntAura].pos.y, g_ameshcylinder[nCntAura].pos.z);
		D3DXMatrixMultiply(&g_ameshcylinder[nCntAura].mtxWorld, &g_ameshcylinder[nCntAura].mtxWorld, &mtxTrans);

		// ワールドマトリックスの設定
		pDevice->SetTransform(D3DTS_WORLD, &g_ameshcylinder[nCntAura].mtxWorld);

		// 頂点バッファをデータストリームに設定
		pDevice->SetStreamSource(0, g_ameshcylinder[nCntAura].pVtxBuff, 0, sizeof(VERTEX_3D));

		// インデックスバッファをデータストリームに設定
		pDevice->SetIndices(g_ameshcylinder[nCntAura].pIdxBuff);
		...
		if (g_ameshcylinder[nCntAura].bUse == true)
		{
			// オーラの描画
...
void UpdateAura(void)
{
	...
	for (int nCntAura = 0; nCntAura < MAX_AURA; nCntAura++, pAura++)
	{
		...
		for (int nCntHeight = 0; nCntHeight < AURA_SPLIT_HEIGTH; nCntHeight++)
		{
			...
			{
				// 頂点座標の設定
				pVtx[nCntVtxWidht].pos = D3DXVECTOR3(sinf(fRot) * (g_ameshcylinder[nCntAura].fRadius - (50.0f * nCntHeight)), -AURA_HEIGHT * nCntHeight, cosf(fRot) * (g_ameshcylinder[nCntAura].fRadius - (50.0f * nCntHeight)));
